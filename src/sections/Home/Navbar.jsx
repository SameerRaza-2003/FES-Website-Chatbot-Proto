    ...
    <nav className="fixed w-full z-40 top-0 left-0 bg-white shadow-md">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between py-5">
          {/* Logo (clickable to go home from other pages) */}
          <div
            className="flex items-center gap-3 cursor-pointer"
            onClick={
              currentPage !== 'home' ? onBackToHome : undefined
            }
          >
          ...
          <div className="md:hidden glass-strong p-4 mt-2 space-y-3">
            {currentPage === 'home' ? (
              <>
                ...
                <button
                  onClick={() => { onCourseFinder(); setOpen(false); }}
                  className="w-full py-2 rounded-full border border-fes-blue text-fes-blue hover:bg-fes-blue hover:text-white transition"
                >
                  Course Finder
                </button>
                <button
                  onClick={() => { onGetStarted(); setOpen(false); }}
                  className="w-full py-2 rounded-full bg-gradient-to-r from-fes-blue to-fes-deep text-white"
                >
                  AI Assistant
                </button>
              </>
              ...
              <button
                onClick={() => { onBackToHome(); setOpen(false); }}
                className="w-full py-2 rounded-full bg-gradient-to-r from-fes-blue to-fes-deep text-white"
              >
                Back to Home
              </button>
